...
module Fluent
  ...
  module ExceptionDetectorConfig
    ...
    DART_RULES = [
      ...
      rule(:dart_type_err_4, /^$/, :dart_stack_begin),
      ...
      rule(:dart_format_err_3, /^$/, :dart_stack_begin),
      ...
      rule(:dart_method_err_3, /^Found:/, :dart_stack_begin),
      rule(:dart_method_err_3, /^#\d+\s+.+?\(.+?\)$/, :dart_stack),
      rule(:dart_exc, /^.*$/, :dart_stack_begin),
      rule(:dart_stack_begin, /^#\d+\s+.+?\(.+?\)$/, :dart_stack),
